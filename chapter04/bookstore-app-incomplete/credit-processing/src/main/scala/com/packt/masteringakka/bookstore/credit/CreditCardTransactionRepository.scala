...
object CreditCardTransactionRepository {
  implicit val GetTxn = GetResult { r =>
    CreditCardTransactionFO(
      r.<<,
      CreditCardInfo(r.<<, r.<<, r.<<, r.nextDate),
      r.<<,
      CreditTransactionStatus.withName(r.<<),
      r.<<,
      r.nextTimestamp,
      r.nextTimestamp,
      false
...
class CreditCardTransactionRepository(implicit ec: ExecutionContext) extends EntityRepository[CreditCardTransactionFO] {
  ...
  def loadEntity(id: Int) = {
    val select =
      sql"select id, cardHolder, cardType, cardNumber, expiration, amount, status, confirmationCode, createTs, modifyTs from CreditCardTransaction where id = $id"
  ...
  def deleteEntity(id: Int) =
    Future.failed(new NotImplementedError("delete not implemented for credit card transaction"))
}
