...
case class BookFO(id: String,
    ...
    extends EntityFieldsObject[String, BookFO] {
...
object Book {
  ...
  object Event {
    case class BookCreated(book: BookFO) extends EntityEvent {
      def toDatamodel = {
        ...
        Datamodel.BookCreated.newBuilder.setBook(bookDM).build
      }
    }
      ...
      def fromDatamodel = {
        case bc: Datamodel.BookCreated =>
          ...
          val book = BookFO(
            ...
            bookDm.getTitle(),
            bookDm.getAuthor(),
            bookDm.getTagList().toList,
            bookDm.getCost(),
            bookDm.getInventoryAmount(),
            new Date(bookDm.getCreateTs()),
            bookDm.getDeleted()
          )
          BookCreated(book)
      }
    ...
    case class TagAdded(tag: String) extends EntityEvent {
      def toDatamodel =
        Datamodel.TagAdded.newBuilder().setTag(tag).build
    }
    ...
    case class TagRemoved(tag: String) extends EntityEvent {
      def toDatamodel =
        Datamodel.TagRemoved.newBuilder().setTag(tag).build
    ...
    case class InventoryAdded(amount: Int) extends EntityEvent {
      def toDatamodel =
        Datamodel.InventoryAdded.newBuilder().setAmount(amount).build
    }
    ...
    case class InventoryAllocated(orderId: String, amount: Int) extends EntityEvent {
      def toDatamodel =
        Datamodel.InventoryAllocated.newBuilder().setOrderId(orderId).setAmount(amount).build
    }
    ...
    case class InventoryBackordered(orderId: String) extends EntityEvent {
      def toDatamodel =
        Datamodel.InventoryBackordered.newBuilder().setOrderId(orderId).build
    }
    ...
    case class BookDeleted(id: String) extends EntityEvent {
      def toDatamodel =
        Datamodel.BookDeleted.newBuilder().setId(id).build
  ...
  val BookAlreadyCreated = ErrorMessage(
    "book.alreadyexists",
    Some("This book has already been created and can not handle another CreateBook request")
  )
  val InventoryNotAvailError =
    ErrorMessage("inventory.notavailable", Some("Inventory for an item on an order can not be allocated"))
}
