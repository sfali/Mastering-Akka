...
object BookstoreUserRepository {
  ...
  implicit val GetUser = GetResult { r =>
    BookstoreUserFO(r.<<, r.<<, r.<<, r.<<, r.nextTimestamp, r.nextTimestamp)
  }
}
...
class BookstoreUserRepository(implicit ec: ExecutionContext) extends EntityRepository[BookstoreUserFO] {
  ...
  def loadEntity(id: Int) =
    db.run(sql"#$SelectFields where id = $id and not deleted".as[BookstoreUserFO]).map(_.headOption)
  ...
  def findUserIdByEmail(email: String) =
    db.run(sql"select id from StoreUser where email = $email and not deleted".as[Int]).map(_.headOption)
