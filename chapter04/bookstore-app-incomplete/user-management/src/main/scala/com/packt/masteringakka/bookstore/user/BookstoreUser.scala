...
case class BookstoreUserFO(id: Int,
                           firstName: String,
                           lastName: String,
                           email: String,
                           createTs: Date,
    ...
    extends EntityFieldsObject[Int, BookstoreUserFO] {
...
class BookstoreUser(idInput: Int) extends EntityActor[BookstoreUserFO](idInput) {
  ...
  override def customCreateHandling: StateFunction = {
    case Event(vo: BookstoreUserFO, _) =>
      ...
      checkFut.map(b => FinishCreate(vo)).to(self, sender())
      stay
  }
  ...
  def emailUnique(email: String, existingId: Option[Int] = None) =
    repo.findUserIdByEmail(email).flatMap {
