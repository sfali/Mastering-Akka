...
abstract class PersistentEntity[FO <: EntityFieldsObject[String, FO]: ClassTag](id: String)
  ...
  def maybeSnapshot: Unit =
    snapshotAfterCount.filter(i => eventsSinceLastSnapshot >= i).foreach { i =>
...
/**
  ...
  */
abstract class Aggregate[FO <: EntityFieldsObject[String, FO], E <: PersistentEntity[FO]: ClassTag]
