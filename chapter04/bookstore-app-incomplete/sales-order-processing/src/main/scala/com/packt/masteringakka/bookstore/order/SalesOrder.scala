...
case class SalesOrderFO(id: String,
                        userId: String,
                        creditTxnId: String,
                        status: SalesOrderStatus.Value,
                        totalCost: Double,
                        lineItems: List[SalesOrderLineItemFO],
                        createTs: Date,
                        deleted: Boolean = false)
    extends EntityFieldsObject[String, SalesOrderFO] {
...
object SalesOrder {
  ...
  object Command {
    case class CreateOrder(newOrderId: String,
                           userEmail: String,
  ...
  object Event {
    case class OrderCreated(order: SalesOrderFO) extends EntityEvent {
      def toDatamodel = {
        ...
        val orderDm = Datamodel.SalesOrder
          .newBuilder()
          .setId(order.id)
        ...
        Datamodel.OrderCreated.newBuilder().setOrder(orderDm).build
      }
    }
    object OrderCreated extends DatamodelReader {
      def fromDatamodel = {
        case doc: Datamodel.OrderCreated =>
          ...
          val order = SalesOrderFO(
            dmo.getId(),
            dmo.getUserId(),
            dmo.getCreditTxnId(),
            SalesOrderStatus.withName(dmo.getStatus()),
            dmo.getTotalCost(),
            items.toList,
            new Date(dmo.getCreateTs())
          )
          OrderCreated(order)
      }
    }
    ...
    case class OrderStatusUpdated(status: SalesOrderStatus.Value) extends EntityEvent {
      def toDatamodel = Datamodel.OrderStatusUpdated.newBuilder().setStatus(status.toString).build
    }
...
private[order] object SalesOrderCreateValidator {
  ...
  case class UnresolvedDependencies(inputs: Inputs,
                                    userAssociate: Option[ActorRef] = None,
                                    invClerk: Option[ActorRef] = None,
                                    creditAssociate: Option[ActorRef] = None)
      extends InputsData

  case class ResolvedDependencies(inputs: Inputs,
                                  expectedBooks: Set[String],
                                  user: Option[BookstoreUserFO],
                                  books: Map[String, BookFO],
                                  userAssociate: ActorRef,
                                  invClerk: ActorRef,
                                  creditAssociate: ActorRef)
      extends InputsData

  case class LookedUpData(inputs: Inputs, user: BookstoreUserFO, items: List[SalesOrderLineItemFO], total: Double)
      extends InputsData
  ...
  val InventoryNotAvailError =
    ErrorMessage("order.inventory.notavailable", Some("Inventory for an item on this order is no longer available"))
}

private[order] class SalesOrderCreateValidator
    extends FSM[SalesOrderCreateValidator.State, SalesOrderCreateValidator.Data] {
  ...
  when(ResolvingDependencies, ResolveTimeout)(transform {
    ...
    case FSM.State(
        state,
        UnresolvedDependencies(inputs, Some(userAssociate), Some(invClerk), Some(creditAssociate)),
        _,
        _,
        _
        ) =>
      ...
      goto(LookingUpEntities) using ResolvedDependencies(
        inputs,
        expectedBooks,
        None,
        Map.empty,
  ...
  when(LookingUpEntities, 10 seconds)(transform {
    ...
    case FSM.State(
        state,
        ResolvedDependencies(inputs, expectedBooks, Some(u), bookMap, userAssociate, invClerk, creditAssociate),
        _,
        _,
        _
      ...
      val lineItems = inputs.request.lineItems.zipWithIndex.flatMap {
        case (item, idx) =>
          bookMap.get(item.bookId).map(b => SalesOrderLineItemFO(idx + 1, b.id, item.quantity, item.quantity * b.cost))
      }
  ...
  when(ChargingCard, 10 seconds) {
    case Event(FullResult(txn: CreditCardTransactionFO), data: LookedUpData)
        if txn.status == CreditTransactionStatus.Approved =>
      val order = SalesOrderFO(
        data.inputs.request.newOrderId,
        data.user.id,
        txn.id,
