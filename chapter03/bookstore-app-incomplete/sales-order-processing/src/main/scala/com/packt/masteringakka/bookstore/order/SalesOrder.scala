...
import com.packt.masteringakka.bookstore.user.UserManager
...
case class SalesOrderLineItemFO(id: Int,
                                orderId: Int,
                                bookId: Int,
                                quantity: Int,
                                cost: Double,
...
case class SalesOrderFO(id: Int,
                        userId: Int,
                        creditTxnId: Int,
    ...
    extends EntityFieldsObject[SalesOrderFO] {
...
object SalesOrder {
  ...
  case class ResolvedDependencies(inputs: Inputs,
                                  ...
                                  books: Map[Int, BookFO],
                                  userAssociate: ActorRef,
                                  invClerk: ActorRef,
                                  creditAssociate: ActorRef)
      extends InputsData

  case class LookedUpData(inputs: Inputs, user: BookstoreUser, items: List[SalesOrderLineItemFO], total: Double)
      extends InputsData
...
class SalesOrder(idInput: Int) extends EntityActor[SalesOrderFO](idInput) {
  ...
  when(LookingUpEntities, 10 seconds)(transform {
    ...
    case FSM.State(
      ...
      val lineItems = inputs.request.lineItems.flatMap { item =>
        bookMap
          .get(item.bookId)
  ...
  when(ChargingCard, 10 seconds) {
    case Event(FullResult(txn: CreditCardTransaction), data: LookedUpData)
      ...
      val order =
        SalesOrderFO(0, data.user.id, txn.id, SalesOrderStatus.InProgress, data.total, data.items, newDate, newDate)
