...
object SalesOrderRepository {
  ...
  implicit val GetOrder = GetResult { r =>
    SalesOrderFO(r.<<, r.<<, r.<<, SalesOrderStatus.withName(r.<<), r.<<, Nil, r.nextTimestamp, r.nextTimestamp)
  }
  implicit val GetLineItem = GetResult { r =>
    SalesOrderLineItemFO(r.<<, r.<<, r.<<, r.<<, r.<<, r.nextTimestamp, r.nextTimestamp)
  }
}
...
class SalesOrderRepository(implicit ex: ExecutionContext) extends EntityRepository[SalesOrderFO] {
  ...
  def findOrderIdsForBookTag(tag: String) = {
    val select =
      sql"select distinct(l.orderId) from SalesOrderLineItem l left join BookTag t on l.bookId = t.bookId where t.tag = $tag"
    db.run(select.as[Int])
  }
