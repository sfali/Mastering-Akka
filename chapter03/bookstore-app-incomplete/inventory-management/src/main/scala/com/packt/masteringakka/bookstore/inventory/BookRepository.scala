...
object BookRepository {
  implicit val GetBook = GetResult { r =>
    BookFO(
      r.<<,
      r.<<,
      r.<<,
...
class BookRepository(implicit ec: ExecutionContext) extends EntityRepository[BookFO] {
  ...
  def findBookIdsByTags(tags: Seq[String]) = {
    ...
    val idsWithAllTags = db.run(
      sql"select bookId, count(bookId) from BookTag where lower(tag) in (#$tagsParam) and not deleted group by bookId having count(bookId) = ${tags.size}"
        .as[(Int, Int)]
    )
  ...
  def persistEntity(book: BookFO) = {
    ...
    def tagsInserts(bookId: Int) =
      DBIOAction.sequence(book.tags.map(t => sqlu"insert into BookTag (bookid, tag) values ($bookId, $t)"))
  ...
  def allocateInventory(id: Int, amount: Int) = {
    val fut =
      db.run(
        sqlu"update Book set inventoryAmount = inventoryAmount - $amount where id = $id and inventoryAmount >= $amount"
      )
    fut.flatMap {
