...
case class BookFO(id: Int,
    ...
    extends EntityFieldsObject[BookFO] {
...
private[inventory] class Book(idInput: Int) extends EntityActor[BookFO](idInput) {
  ...
  def initializedHandling: StateFunction = {
    ...
    case Event(AddInventory(amount: Int), InitializedData(fo: BookFO)) =>
      requestFoForSender
      persist(fo, repo.addInventoryToBook(fo.id, amount), _ => fo.copy(inventoryAmount = fo.inventoryAmount + amount))
