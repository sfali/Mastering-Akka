...
object UserManager {
  ...
  val EmailNotUniqueError =
    ErrorMessage("user.email.nonunique", Some("The email supplied for a create or update is not unique"))
}
...
class UserManager extends BookstoreActor {
  ...
  def emailUnique(email: String, existingId: Option[Int] = None) =
    dao.findUserByEmail(email).flatMap {
...
object UserManagerDao {
  ...
  implicit val GetUser = GetResult { r =>
    BookstoreUser(r.<<, r.<<, r.<<, r.<<, r.nextTimestamp, r.nextTimestamp)
  }
}
...
class UserManagerDao(implicit ec: ExecutionContext) extends BookstoreRepository {
  ...
  def findUserById(id: Int) =
    db.run(sql"#$SelectFields where id = $id and not deleted".as[BookstoreUser]).map(_.headOption)
  ...
  def findUserByEmail(email: String) =
    db.run(sql"#$SelectFields where email = $email and not deleted".as[BookstoreUser]).map(_.headOption)
