name := "chapter3-bookstore-app-incomplete"

lazy val commonSettings = Seq(organization := "com.packt.masteringakka", version := "0.1.0", scalaVersion := "2.11.8")

lazy val root =
  (project in file(".")).aggregate(common, inventoryMgmt, userMgmt, creditProcessing, salesOrderProcessing, server)

lazy val common = (project in file("common")).settings(commonSettings: _*)

lazy val inventoryMgmt = (project in file("inventory-management")).settings(commonSettings: _*).dependsOn(common)

lazy val userMgmt = (project in file("user-management")).settings(commonSettings: _*).dependsOn(common)

lazy val creditProcessing = (project in file("credit-processing")).settings(commonSettings: _*).dependsOn(common)
